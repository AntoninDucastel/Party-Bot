...
import command.defaut.CommandDefault;
import command.defaut.HelpCommand;
import event.MainBot;
import music.MusicCommand;
...
public final class CommandMap {
    ...
    public CommandMap(MainBot mainBot) {
    	...
    	registerCommands(new CommandDefault(mainBot, this), new HelpCommand(this), new MusicCommand());
    	load();
    }
    
    
    private void load() {
    	File file = new File("UsePower.json");
...
JSONArray array = new JSONArray();
    ...
    public int getPowerUser(Guild guild, User user) {
    	if (guild.getMember(user).hasPermission(Permission.ADMINISTRATOR)) return 10;
    ...
    public void registerCommand(Object object){
        for(Method method : object.getClass().getDeclaredMethods()){
            if(method.isAnnotationPresent(Commandes.class)){
                Commandes command = method.getAnnotation(Commandes.class);
    ...
    private void execute(SimpleCommand simpleCommand, String command, String[] args, Message message) throws Exception{
        ...
        for(int i = 0; i < parameters.length; i++){
            ...
            else if(parameters[i].getType() == JDA.class) objects[i] = mainBot.getJda();
